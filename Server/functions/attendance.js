    ...
    return new Promise(async (resolve, reject) => {
        dbpool.getConnection((err, conn) => {
            if (err) {
                ...
                conn.query({
                    sql: 'SELECT enrollments.*, user_profile.displayName FROM enrollments JOIN user_profile ON enrollments.User_profile_Id = user_profile.ID JOIN users ON user_profile.userId = users.id WHERE users.id = ?;',
                    timeout: 40000,
                    values: [data.userId]
                }, (error, results) => {
                    if (error) {
